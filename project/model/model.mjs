...
const connection = mysql.createConnection({
    ...
    async function UpdateEmail(email,emailToChange,req,res) {
      ...
      try{
       ...
       //const hashNewPass = await bcrypt.hash(password, 10);
        const update_prof = "UPDATE visitor SET email=? WHERE email=?";
        const result_tick = await connection.promise().query(update_prof, [emailToChange,email]);
        req.session.email=emailToChange
...
/*
    async function UpdateProfile(email,emailToChange, req,res) {
...
export {insertDataIntoTable,UserValidationRegister,UserValidationLogin,insertUserData,getExhibitions,update,UpdateEmail};
